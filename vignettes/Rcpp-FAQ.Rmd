...
# For footer text
lead_author_surname: Eddelbuettel and Francois
...
non-standard code in their \verb|~/.Rprofile| or `Rprofile.site` (or
equivalent) files.
...
## Can I do matrix algebra with \pkg{Rcpp}? {#matrix-algebra}
...
- Pre-defined constants including:
    ...
    - Null Values: `R_NilValue`, `NA_STRING`,
...
- Matrix types instantiated using the rows, cols constructor `Rcpp::<Type>Matrix n(rows,cols)`
...
\pkg{Rcpp} objects are wrappers around the underlying \Rs objects' `SEXP`,
...
That is to say, the `SEXP` does \textit{not} hold the actual data of the
...
use the same `SEXP` that powers the original \Rs object if the types match
...
the new \pkg{Rcpp} object that uses the same `SEXP` for the \Rs object.
...
`SEXP` object. Therefore, the initially secure \Rs object would be altered.
...
Prior to \R 's 3.0.0, the largest vector one could obtain was at most $2^{31} - 1$
elements. With the release of \R 's 3.0.0, long vector support was added to
...
use of `Rcpp::internal::string_proxy`. In particular,
`Rcpp::internal::string_proxy` is \textit{not} MoveAssignable since the
left hand side of `operator=(const string_proxy \&rhs)` is \textit{not}
...
using `CharacterVector` with `std::swap`, `std::move`,
...
\textit{not} have access to it within its implementation of `StrCmp`.
