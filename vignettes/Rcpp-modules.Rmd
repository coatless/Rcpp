---
title: |
       | Exposing \proglang{C++} functions and classes with \pkg{Rcpp} modules
...
A Rcpp module is created in a `cpp` file using the `RCPP\_MODULE`
...
A class is exposed using the `class\_` keyword. The `Uniform`
...
`class\_` is templated by the \proglang{C++} class or struct
that is to be exposed to \proglang{R}.
The parameter of the `class\_<Uniform>` constructor is the name we will
...
to the R level using the `.constuctor` template method of `.class\_`.
...
`class\_` has three ways to expose fields and properties, as
...
The `.field\_readonly` exposes a public field with read-only access from R.
...
as `set\_z` above, or a free function taking a pointer to the target
class and a \textbf{T} :
...
`class\_` has several overloaded and templated `.method`
...
\paragraph{Documenting methods} `.method` can also include
...
To resolve the ambiguity, it is possible to use `const\_method`
or `nonconst\_method` instead of `method` in order
...
The `.finalizer` member function of `class\_` can be used to
...
When a \proglang{C++} class is exposed by the `class\_` template,
