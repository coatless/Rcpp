...
in \pkg{Rcpp}---and the need for explicit `return` and
`Rcpp::wrap` statements, the code is now identical between
...
One important thing to note concernc the conversion to the `bool`
...
`seq_len` creates an integer sugar expression whose \ith\
element expands to `i`. `seq_len` is particularly useful in
